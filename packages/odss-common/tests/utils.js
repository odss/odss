...
QUnit.test('prepare function name', assert =>  {
    ...
    assert.equal(getTokenType(functionWithName), 'functionWithName', 'Service name as funtion name');

    let namespaceFunction = function a_b_c_d_NamespaceFunction() {};
    assert.equal(getTokenType(namespaceFunction), 'a.b.c.d.NamespaceFunction', 'Service name as funtion name with namespace');

    function SomeFunction() {}
    SomeFunction.$namespace = 'a.b.c.d';
    assert.equal(getTokenType(SomeFunction), 'a.b.c.d.SomeFunction', 'Service name with custom namespace');
});
