...
import { HandlerTypes, getFactoryContext, BindHandlerType, ProvideHandlerType,
    PropertiesInjectHandlerType, ConstructorInjectHandlerType, PropertyHandlerType
...
export function Inject(token?: any, filter: any = null) {
    return (target: any, key: string | symbol, index?: number): void => {
        ...
        if (index === undefined) {
            // properties
            getFactoryContext(target.constructor)
                .setDefaults<PropertiesInjectHandlerType[]>(
